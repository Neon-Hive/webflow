  ...
  Dropdown.animations = {
    navigation: {
        ...
        mm.add("(max-width: 59.9em)", () => {
          ...
          if (megaFill) {
            ...
            timeline.fromTo(
              megaFill,
              { height: '0%' },
              { height: '100%', duration: 0.1, ease: 'inOutCubic' }, '0'
            );
          }
        });
      ...
      close(list, onComplete) {
        ...
        mm.add("(max-width: 59.9em)", () => {
          // Small screen reverse animations
          if (megaFill) {
            timeline.fromTo(
              megaFill,
              { opacity: 1 },
            ...
            timeline.fromTo(
              megaFill,
              { height: '100%' },
              { height: '0%', duration: 0.1, ease: 'inOutCubic' }, '-=0.1'
            );
          }
        });
  
        mm.add("(min-width: 60em)", () => {
          // Large screen reverse animations
          if (megaFill) {
            timeline.fromTo(
              megaFill,
              { height: '100%' },
...
pageFunctions.addFunction('navigationDropdownVisual', function() {
  ...
  const navigationProjects = document.querySelectorAll("[data-navigation-projects]");
  ...
  navigationProjects.forEach(project => {
    ...
    links.forEach((link, index) => {
      ...
      link.addEventListener("mouseleave", () => {
        ...
        const anyHovered = Array.from(links).some(link => link.classList.contains("is-active"));
