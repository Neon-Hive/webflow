...
pageFunctions.addFunction("splitGlobalAnimation", function () {
    ...
    document.querySelectorAll("[data-split-gsap='words']").forEach((target) => {
        ...
        const elementsToSplit = target.querySelectorAll("p, h1, h2, h3, h4, h5, h6");
        const splitTargets = elementsToSplit.length > 0 ? elementsToSplit : [target];
...
pageFunctions.addFunction("globalGSAP", function () {
            ...
            switch (animationType) {
                case "fade":
                    timeline.fromTo(
                        element,
                        { opacity: 0 },
                        { opacity: 1, duration, delay, ease: "power1.in" }
                    );
                    break;
                ...
                case "fade-in":
                    timeline.fromTo(
                        element,
                        { opacity: 0, y: "2rem" },
                        { opacity: 1, y: "0rem", duration, delay, ease: "power2.inOut" }
                    );
                    break;
                ...
                case "from-top":
                    timeline.fromTo(
                        element,
                        { opacity: 0, y: "-100%" },
                        { opacity: 1, y: "0%", duration, delay, ease: "power2.in" }
                    );
                    break;
                ...
                case "scale-in":
                    timeline.fromTo(
                        element,
                        { opacity: 0, scale: 0.8 },
                        { opacity: 1, scale: 1, duration, delay, ease: "power1.out" }
                    );
                    break;
                ...
                case "border-width":
                    timeline.fromTo(
                        element,
                        { opacity: 0, width: "0%" },
                        { opacity: 1, width: "100%", duration, delay, ease: "power1.inOut" }
                    );
                    break;
                ...
                case "border-height":
                    timeline.fromTo(
                        element,
                        { opacity: 0, height: "0%" },
                        { opacity: 1, height: "100%", duration, delay, ease: "power1.inOut" }
                    );
                    break;
    ...
    const visualScaleIn = document.querySelectorAll("[data-animation-img=scale-in]");
    if (visualScaleIn.length > 0) {
        visualScaleIn.forEach((element) => {
            ...
            gsap.fromTo(
                visual,
                { yPercent: 20, scale: 0.8, rotate: 5, opacity: 0 },
                {
                    yPercent: 0,
                    scale: 1,
                    rotate: 0,
    ...
    const visualParallexSmall = document.querySelectorAll("[data-animation-img=visual-parallex-small]");
