...
tlTransition.to({}, {
  duration: 2, // Adjust this duration to match your Lottie animation length
  onUpdate: function() {
    const progress = Math.min(this.progress(), 0.5);
    // const roundedProgress = Math.round(progress * 20) * 5; // Round to nearest 5%
    
    // // Only log when we hit a new 5% milestone
    // if (this._lastLoggedProgress !== roundedProgress) {
    //   console.log(`Lottie progress: ${roundedProgress}%`);
    //   this._lastLoggedProgress = roundedProgress;
    // }
...
tlTransition.to(".transition_wrap", {
  ...
  duration: 1.2, // Adjusted duration for clip-path animation
  ease: "quartInOut",
}, "-=0.5");
