...
const lenis = new Lenis({
  ...
  touchMultiplier: 1.2, // Touch scroll sensitivity
...
function initScrambleOnHover(){
    document.querySelectorAll('[data-button-hover="text"]').forEach((wrap) => {
      const trigger = wrap.querySelector('.g_clickable_wrap, .g_clickable_link, .g_clickable_btn') || wrap;
      const textEl  = wrap.querySelector('[data-button-text="target"]');
      if (!textEl) return;
  
      const orig = textEl.textContent;
      const w = textEl.getBoundingClientRect().width || textEl.offsetWidth || (orig.length * 0.6 * 16);
      ...
      trigger.addEventListener('mouseenter', () => {
        ...
        gsap.to(textEl, {
          duration: 1,
          scrambleText: { text: orig, chars: '01', tweenLength: false },
      ...
      trigger.addEventListener('mouseleave', () => {
        ...
        gsap.to(textEl, {
          duration: 0.6,
          scrambleText: { text: orig, speed: 2, chars: '01', tweenLength: false },
          onComplete: () => (busy = false)
        });
      });
    });
