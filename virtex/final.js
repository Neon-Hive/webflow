...
pageFunctions.addFunction('pageTransition', function() {
  ...
  const transitionLottie = lottie.loadAnimation({
    ...
    loop: false, 
    autoplay: false, 
    path: 'https://cdn.prod.website-files.com/680f5fbcc4cb7ef827577478/6818bc7677c2d7bb33dcd3aa_57bd411151fb506e49da4ab4ed6a2916_Virtex%20Intro.json' 
  });
  ...
  // Set initial states
  tlTransition.set(".transition_wrap", {
    display: "flex",
    willChange: "clip-path",
    clipPath: "polygon(0% 0%, 100% 0%, 100% 100%, 0% 100%)",
  });
  
  // First play the Lottie animation (0 to 99%)
  tlTransition.to({}, {
    delay: 0.2,
    duration: 3, // Adjust this duration to match your Lottie animation length
    onUpdate: function() {
      const progress = Math.min(this.progress(), 0.5);   
      transitionLottie.goToAndStop(Math.floor(transitionLottie.totalFrames * progress), true);
    },
  });
  
  // Then animate the clip path
  tlTransition.to(".transition_wrap", {
  ...
  function runLaunchPageAnimation() {
    ...
    const headerBody2Text = headerBody2Wrap?.querySelector('.header_btn_layout');
    ...
    function setupScrollAnimation() {
      ...
      // Set headerBody2Wrap to pointer-events: none initially
      if (headerBody2Wrap) {
        gsap.set(headerBody2Wrap, { 
          pointerEvents: "none" 
        });
      }
      ...
      if (headerBody2Wrap) {
        // When headerBody2Wrap starts animating, update pointer-events to auto
        headerAnimation.to(headerBody2Wrap, {
          opacity: 1,
          pointerEvents: "auto", // Add this line to update pointer-events
          duration: 0.05
        }, 0.5);
  ...
  function runGlobalAnimation() {
    ...
    if (pageVisual) {
      gsap.set(pageVisual, { visibility: 'visible' });
    }
  
    if (pageFade) {
      gsap.set(pageFade, { visibility: 'visible' });
    }
  ...
  function runErrorAnimation() {
    ...
    if (pageBg) {
      gsap.set(pageBg, { visibility: 'visible' });
    }
  ...
  pageFunctions.addFunction("globalAnimation", function () {
    ...
    function initScrambleOnHover(){
      let targets = document.querySelectorAll('[data-button-hover="text"]')

      targets.forEach((target) => {
        let textEl = target.querySelector('[data-button-text="target"]')
        let originalText = textEl.textContent // save original text

        // Set initial styles to prevent jumping
        textEl.style.display = 'inline-block';
        textEl.style.minWidth = `${textEl.offsetWidth}px`;

        // Create a variable to track if animation is in progress
