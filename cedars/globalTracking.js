...
// Categories that represent article verticals
	...
	if (!isArticleDetail()) {
		...
		adobeDataLayer.push({
			...
			navigation: {
				...
				siteSubsection4:
					location.pathname.replace(/^\/+/, "").split("/")[5] || "",
				pageURL: location.href,
				refURL: document.referrer || "",
			},
			pageName:
				"cs-org:cedars-sinai:" +
				location.pathname.substring(1).replaceAll("/", ":").split(".")[0],
...
function handleAnalyticsClick(event) {
	...
	const componentName = clickedElement
		.closest("[data-component-name]")
		?.getAttribute("data-component-name");
	const componentTitle =
		clickedElement
			.closest("[data-component-title]")
			?.getAttribute("data-component-title") || "N/A";
	...
	const io = new IntersectionObserver(
		(ents) => {
			ents.forEach(({ target, isIntersecting }) => {
				...
				const name =
					target.getAttribute("data-form-name") ||
					target.name ||
					target.id ||
					"unnamed-form";
...
function maskValueIfPII(name, type, value, maskPII) {
	...
	const looksEmail =
		/email/.test(n) || t === "email" || /\S+@\S+\.\S+/.test(value);
...
function attachFormTracking(
	...
	forms.forEach((form) => {
		const derivedFormName =
			form.getAttribute(formNameAttr) ||
