...
function combineItems() {
	...
	targets.forEach((target) => {
		...
		const maxItems = Number.parseInt(
			target.getAttribute("n4-combine-max") || "999",
			10,
		);

		const sources = document.querySelectorAll(
			`[n4-combine-source="${targetName}"]`,
		);

		if (sources.length === 0) return;

		const existingItems = Array.from(
			target.querySelectorAll(".w-dyn-item"),
		...
		const paginationWrapper = document.querySelector(
			`[n4-combine-pagination="${targetName}"]`,
		);

		console.log(
			`Pagination check for "${targetName}": uniqueItems=${uniqueItems.length}, maxItems=${maxItems}, paginationWrapper=${!!paginationWrapper}`,
		);

		if (paginationWrapper) {
			const updatePaginationVisibility = () => {
				console.log(
					`Checking pagination: uniqueItems.length=${uniqueItems.length}, maxItems=${maxItems}, shouldShow=${uniqueItems.length > maxItems}`,
				);
			...
			setTimeout(updatePaginationVisibility, 2000);
		} else {
			console.warn(`Pagination wrapper not found for "${targetName}"`);
		}
...
function showPage(target, pageNumber) {
	const { allItems, itemsPerPage, totalPages, paginationWrapper } =
		target._paginationData;
