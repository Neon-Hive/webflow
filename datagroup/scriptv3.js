...
(function () {
  ...
  pageFunctions.addFunction('pageTransition', function () {
    ...
    // Page Reveal (only on first load)
    ...
    // Always hide transition wrap initially to prevent flicker
    gsap.set(transitionWrap, { display: "none", visibility: "hidden" });
    
    if (isFirstLoad) {
      // Mark that the user has visited
      sessionStorage.setItem('hasVisited', 'true');
      
      // Show transition wrap for first load only
      gsap.set(transitionWrap, { display: "flex", visibility: "visible" });
      ...
      // Skip page reveal animation - transition wrap already hidden
      runAnimation();
    }
    ...
    function runHeaderSecondaryPageAnimation() {
      ...
      if (headerText.length > 0) {
        ...
        tl.fromTo(
          headerText,
          { opacity: 0 },
          { opacity: 1, duration: 0.9, stagger: 0.2, ease: 'sine.out' },
          "-=0.1"
        );
      }
    ...
    // Page-specific animation for Header Third page
...
document.querySelectorAll("[data-calendly='true']").forEach(button => {
  button.addEventListener("click", function () {
    Calendly.initPopupWidget({ url: "https://calendly.com/graeve/30min?hide_gdpr_banner=1" });
  });
});
});
