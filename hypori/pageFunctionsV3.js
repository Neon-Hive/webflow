...
pageFunctions.addFunction("breadcrumbLine", function () {
  ...
  const productURLs = ["/how-it-work", "/use-cases", "/pricing"];
  ...
  const resourcesURLs = [
    ...
    "/learning-center",
    "/resource-library",
    "/resource-library-locked/",
    "/resources/dod-faqs",
  ];
  const contactURLs = ["/contact", "/contact#support"];

  // Regex for dynamic use-cases paths
  const dynamicUseCasesRegex = /^\/use-cases\/.+$/;

  // Helper function to activate matching breadcrumb line
  function activateLine(urlArray, line, regex) {
    if (
      urlArray.some((path) => window.location.pathname.includes(path)) ||
  ...
  breadcrumbLines.each(function () {
    ...
    if (lineType === "products") {
      activateLine(productURLs, line, dynamicUseCasesRegex);
