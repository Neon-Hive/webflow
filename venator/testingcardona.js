...
pageFunctions.addFunction('parallexSection', function() {
  // Store ScrollTrigger instances to refresh later
  const scrollTriggers = [];
  
  // Function to initialize all animations
  function initAnimations() {
    // Clear any existing ScrollTriggers first to prevent duplicates
    ...
    parallaxSections.forEach(section => {
      ...
      scrollTriggers.push(columnsTl.scrollTrigger);
      ...
      if (visualInner) {
        ...
        // Store the ScrollTrigger instance
        scrollTriggers.push(finalTrigger);
      }
    });
  }
  ...
  window.addEventListener('resize', function() {
    // Debounce the resize event to prevent excessive refreshes
    clearTimeout(resizeTimeout);
    resizeTimeout = setTimeout(function() {
      // Option 1: Refresh all ScrollTrigger instances
      ScrollTrigger.refresh();
      
      // Option 2: For more control, reinitialize everything
      // initAnimations();
...
pageFunctions.addFunction('navClassToggle', function() {
  // Script to add is-scrolled class once you scroll past 50px
  const navElement = document.querySelector('.nav_component');
  
  console.log('Nav class toggle component initialized', { navElement });

  function handleClassToggle() {
    const scrollTop = window.pageYOffset || document.documentElement.scrollTop;
    console.log('Class toggle check', { scrollTop });
    
    // Add/remove class based on 50px threshold
    if (scrollTop > 50) {
      navElement.classList.add('is-scrolled');
      console.log('Added is-scrolled class at', scrollTop);
    } else {
      navElement.classList.remove('is-scrolled');
      console.log('Removed is-scrolled class at', scrollTop);
    }
  }

  // Set up the scroll trigger using GSAP
  const scrollTrigger = ScrollTrigger.create({
    start: "top top",
    end: "bottom bottom",
    onUpdate: handleClassToggle,
    markers: false
  });
  console.log('Class toggle ScrollTrigger created', scrollTrigger);

  // Initialize the nav state based on initial scroll position
  handleClassToggle();
  console.log('Initial class toggle executed');

  // Return a cleanup function
  return function cleanup() {
    console.log('Class toggle cleanup function called');
    if (scrollTrigger) {
      scrollTrigger.kill();
      console.log('Class toggle ScrollTrigger killed');
    }
  };
});
